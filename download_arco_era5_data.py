...
def download_arco_data(
    ...
    try:
        arco = ARCO(cache=True, verbose=False, async_timeout=3600)
    ...
    try:
        ...
        filename = f"arco_data_start_{start_date.strftime('%d_%B_%Y')}_end_{end_date.strftime('%d_%B_%Y')}_{end_date.strftime('%d_%B_%Y')}.npy"
...
if __name__ == "__main__":
    ...
    variables = VARIABLES
    download_dir = os.path.join(BASE_SCRATCH_PATH, "arco_data")
    
    try:
        # Setup logging first
        download_frames_dir = os.path.join(BASE_SCRATCH_PATH, "download_frames")
        os.makedirs(download_frames_dir, exist_ok=True)
        logger = setup_logging(download_frames_dir)
