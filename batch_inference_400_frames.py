...
config = {
    ...
    "output_path": f"/scratch/gilbreth/{username}/fcnv2/output/march_13_final_proposal",
    "output_frequency": 1,
...
start_time = datetime.strptime( config["weather_event"]["properties"]["start_time"], "%Y-%m-%d %H:%M:%S").strftime("%d_%B_%Y")
...
def plt_acc(acc_numpy_arr, st_time=start_time, fld="z500", default_timedelta=6, start_year=2020):
    
    # Compute the total number of hours based on the array shape and default timedelta
    total_hours = acc_numpy_arr.shape[0] * default_timedelta
    plt.plot( range(0, total_hours, default_timedelta) , acc_numpy_arr, "-", lw=1, label='Anomaly Correlation Coefficient (ACC) value')
...
plt_acc(acc_numpy_arr, st_time=start_time, fld="z500")
